...
# shared root for the global sweep bookkeeping
GLOBAL_SWEEP_ROOT = Path(pibob.SAVE, "results", "global_shared_sweep")
...
def run_id_from_configuration(configuration) -> str:
    """
    Important:
    Problem is intentionally NOT part of this ID.
    Problem separation is handled only by folders.
    """
...
def done_path(prob: str, configuration: dict) -> Path:
    ...
    return paths["done"] / f"cfg_{run_id}.json"


def claim_path(prob: str, configuration: dict) -> Path:
    paths = get_problem_paths(prob)
    run_id = run_id_from_configuration(configuration)
    return paths["claims"] / f"cfg_{run_id}.claim"


def fail_path(prob: str, configuration: dict) -> Path:
    paths = get_problem_paths(prob)
    run_id = run_id_from_configuration(configuration)
    return paths["fail"] / f"cfg_{run_id}.txt"


def result_trace_path(prob: str, configuration: dict) -> Path:
    paths = get_problem_paths(prob)
    run_id = run_id_from_configuration(configuration)
    return paths["results"] / f"results_{run_id}.csv"


def result_model_path(prob: str, configuration: dict) -> Path:
    paths = get_problem_paths(prob)
    run_id = run_id_from_configuration(configuration)
    return paths["results"] / f"modelresults_{run_id}.csv"


def try_claim(prob: str, configuration: dict) -> bool:
    """
    Atomic file-based claim inside the problem-specific claim folder.
    This ensures the same config for the same problem is only run once.
    """
...
def build_all_jobs(problem_names):
    ...
    Each job is (prob, key, configuration).
    """
    jobs = []
...
def worker_steal_loop(worker_id: int, jobs):
    """
    Workers steal from the global pool of all jobs across all problems.
    """
    ...
    while True:
        ...
        for prob, key, configuration in jobs:
            ...
            did_work = True
            print(f"[worker {worker_id}] CLAIMED problem={prob} key={key}", flush=True)

            try:
                out = run_one_config(prob, key, configuration)
                done_path(prob, configuration).write_text(json.dumps(out, indent=2))
                print(f"[worker {worker_id}] DONE problem={prob} key={key}", flush=True)
...
if __name__ == "__main__":
    run_all_problems(PROBLEM_NAMES, cores_per_run=CORES_PER_RUN)
